...
import { Bitcoin, ArrowUpRight, ArrowDownRight, Send, Wallet, Eye, EyeOff } from 'lucide-react';
...
export function CryptoWallet({ isDuressMode }: CryptoWalletProps) {
  const [modalType, setModalType] = useState<'send' | 'receive' | 'buy' | 'swap' | null>(null);
    ...
    <>
      <section className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-8">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {hiddenBalances.has(asset.id) ? '••••••' : `${asset.balance} ${asset.symbol}`}
                  </p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <div className="text-right">
                  <p className="font-medium text-gray-800">
                    {hiddenBalances.has(asset.id) ? '••••••' : `R ${asset.value.toLocaleString('en-ZA')}`}
                  </p>
        ...
        <div className="grid grid-cols-4 gap-2">
          <QuickAction icon={Send} label="Send" onClick={() => setModalType('send')} />
          <QuickAction icon={ArrowDownRight} label="Receive" onClick={() => setModalType('receive')} />
          <QuickAction icon={Bitcoin} label="Buy" onClick={() => setModalType('buy')} />
          <QuickAction icon={Wallet} label="Swap" onClick={() => setModalType('swap')} />
        </div>
      </section>
